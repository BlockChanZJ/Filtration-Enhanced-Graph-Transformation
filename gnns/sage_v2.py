...
from torch_geometric.nn import SAGEConv, global_add_pool, global_mean_pool, global_max_pool
import argparse
import igraph as ig
import time
...
class SAGE(torch.nn.Module):
    ...
    def __init__(self, d_in, d_hidden, d_out, n_layers, pooling):
        ...
        for layer in range(n_layers):
            if layer == 0:
                self.convs.append(SAGEConv(d_in, d_hidden))
            else:
                self.convs.append(SAGEConv(d_hidden, d_hidden))
    ...
    def forward(self, x, edge_index, batch):
        ...
        for layer in range(self.n_layers):
            ...
            # difference dropout
            x_all += F.dropout(self.pooling(x, batch),p=0.5,training=self.training)
        x = F.relu(self.linear1(x_all))
        x = self.linear2(x)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--run',default=0,type=int,help='[0, 4] (used for 5fold)')
    ...
    model_name = '{}-{}-{}'.format(pooling, n_layers,
                                      'L' if use_layer_feature else 'NL')
    ...
    if use_layer_feature:
        model = SAGE(d_in=d_in, d_hidden=hidden_dim,
                    d_out=n_classes, n_layers=n_layers, pooling=pooling).to(device)
    else:
        model = SAGE(d_in=d_in, d_hidden=hidden_dim, d_out=n_classes,
                    n_layers=n_layers, pooling=pooling).to(device)
    loss_fn = nn.NLLLoss()
